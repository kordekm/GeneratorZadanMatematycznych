...
export function calculateLayoutMetrics(config, maxLinesInTasks) {
...
export function paginateTasks(tasks, tasksPerPage) {
    const pages = [];
    if (tasksPerPage <= 0) {
...
export function getTaskLineCount(task) {
...
export function getMaxLineCount(tasks) {
    if (tasks.length === 0)
        return 2;
