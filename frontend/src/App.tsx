  ...
  const handleImportConfig = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setConfig(prev => ({
        ...prev,
        ...imported,
        addition: { ...prev.addition, ...imported.addition },
        subtraction: { ...prev.subtraction, ...imported.subtraction },
        multiplication: { ...prev.multiplication, ...imported.multiplication },
        division: { ...prev.division, ...imported.division }
      }));
    } catch (err) {
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <label className="px-4 py-3 bg-white/10 backdrop-blur-sm border border-white/20 text-white rounded-lg hover:bg-white/20 text-sm transition cursor-pointer no-print">
                📤 Importuj JSON
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              <button
                ...
                className="px-4 py-3 bg-white/10 backdrop-blur-sm border border-white/20 text-white rounded-lg hover:bg-white/20 text-sm transition no-print"
              >
                {saveStatus === 'saving' ? '⏳ Zapisywanie...' : saveStatus === 'saved' ? '✅ Zapisano!' : saveStatus === 'error' ? '❌ Błąd' : '💾 Zapisz na serwerze'}
              </button>
            </div>
          </div>
        </div>
