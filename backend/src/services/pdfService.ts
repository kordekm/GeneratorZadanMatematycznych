...
export function renderTasksToHtml(tasks: Task[], config: Config): string {
    ...
    for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
        ...
        pagesHtml += `<div style="width: ${metrics.pageWidth * 0.75}px; min-height: ${metrics.pageHeight * 0.75}px; padding: 30px; box-sizing: border-box; ${pageBreak}">`;
        pagesHtml += `<div style="display: grid; grid-template-columns: repeat(${config.columns}, 1fr); gap: 2rem;">${tasksHtml}</div>`;
        pagesHtml += '</div>';
    }
    ...
    try {
        ...
        for (const browser of browsers) {
            try {
                ...
                await execAsync(`${browser} --headless --disable-gpu --no-sandbox --no-pdf-header-footer --print-to-pdf-no-header --print-to-pdf="${outputPath}" "file://${tmpHtmlPath}"`);
